export const CNY_BACKFILL_SYMBOL = "CNY=X";

export type CnyBackfillObservation = {
  date: Date;
  value: number;
};

export type CnyBackfillIndicator = {
  id: number;
  symbol: string;
};

export type BackfillCnyStore = {
  findObservation: (
    indicatorId: number,
    date: Date,
  ) => Promise<{
...
export type CnyBackfillReport = {
  symbol: typeof CNY_BACKFILL_SYMBOL;
  inserted: number;
  updated: number;
  skipped: number;
  firstDate: Date | null;
...
function validObservation(observation: CnyBackfillObservation): boolean {
  return observation.date instanceof Date && !Number.isNaN(observation.date.getTime()) && Number.isFinite(observation.value);
}

export async function backfillCnyObservations(input: {
  indicator: CnyBackfillIndicator;
  observations: CnyBackfillObservation[];
  store: BackfillCnyStore;
}): Promise<CnyBackfillReport> {
  if (input.indicator.symbol !== CNY_BACKFILL_SYMBOL) {
    throw new Error(`CNY backfill only supports ${CNY_BACKFILL_SYMBOL}. Received ${input.indicator.symbol}.`);
  }
