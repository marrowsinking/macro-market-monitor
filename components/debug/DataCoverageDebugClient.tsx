...
function statusTone(status: CoverageStatus): "green" | "teal" | "amber" | "red" | "gray" {
  if (status === "fresh") return "green";
  if (status === "carried_forward") return "teal";
  if (status === "decaying" || status === "insufficient") return "amber";
...
function MetricCard({ label, value, status }: { label: string; value: number; status?: CoverageStatus }) {
  return (
    <div className={`rounded-lg border p-3 ${status ? badgeClass(status) : "border-white/10 bg-white/[0.03] text-slate-100"}`}>
...
function formatDecay(value: number): string {
      ...
      {payload ? (
        <>
          <section className="grid gap-3 md:grid-cols-4 xl:grid-cols-9">
            ...
            <MetricCard label="Fresh" value={payload.summary.freshCount} status="fresh" />
            <MetricCard label="Carried Forward" value={payload.summary.carriedForwardCount} status="carried_forward" />
            <MetricCard label="Decaying" value={payload.summary.decayingCount} status="decaying" />
          ...
          <section className="grid gap-3 text-xs text-slate-300 md:grid-cols-4">
            <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/10 p-3">
              <div className="font-semibold text-emerald-200">Fresh</div>
              <p className="mt-1 text-emerald-100/80">資料在正常更新期內。</p>
            </div>
            <div className="rounded-lg border border-cyan-500/20 bg-cyan-500/10 p-3">
              <div className="font-semibold text-cyan-200">Carried Forward</div>
              <p className="mt-1 text-cyan-100/80">低頻資料仍在合理結轉期內。</p>
            </div>
            <div className="rounded-lg border border-amber-500/20 bg-amber-500/10 p-3">
              <div className="font-semibold text-amber-200">Decaying</div>
              <p className="mt-1 text-amber-100/80">低頻資料可用，但時效價值下降。</p>
            </div>
            <div className="rounded-lg border border-rose-500/20 bg-rose-500/10 p-3">
              <div className="font-semibold text-rose-200">Stale</div>
              <p className="mt-1 text-rose-100/80">明顯過期，才算高風險資料問題。</p>
            </div>
          </section>
          ...
          <section className="rounded-xl border border-white/10 bg-ink-900/70 p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              >
                {["all", "missing", "insufficient", "stale", "decaying", "carried_forward", "fresh", "derived", "placeholder", "not_scored"].map((status) => (
                ...
                <tbody className="divide-y divide-white/10">
                  {rows.map((row) => (
                    <tr key={row.symbol} className="text-slate-300">
                      ...
                      <td className="px-3 py-2"><StatusBadge status={row.freshnessStatus} /></td>
                      <td className="px-3 py-2">{formatDecay(row.decayFactor)}</td>
                      ...
                      <td className="max-w-[360px] px-3 py-2 text-slate-500">{row.freshnessMessage}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
